...
BG_COLOR = (240, 255, 240)  # Light green tint
HEADER_COLOR = (22, 163, 74)  # Green


class MathGameScene(Scene):
    """Math game where kids practice calculations."""
    ...
    def render(self, screen: pygame.Surface) -> None:
        ...
        if self._font:
            title = self._font.render("Rechenspiel", True, WHITE)
            screen.blit(title, (SCREEN_WIDTH // 2 - title.get_width() // 2, 25))

            info = self._font.render("12 + 5 = ?", True, (60, 60, 60))
            screen.blit(
