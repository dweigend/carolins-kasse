    ...
    def test_seed_database_includes_inventoried_products_and_aliases(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with isolated_database_module(db_path) as database:
                ...
                chocolate_ice_cream = database.get_product("1000000000412")
                peas = database.get_product("1000000000443")
            ...
            self.assertIsNotNone(chocolate_ice_cream)
            self.assertEqual(chocolate_ice_cream.name_de, "Schokoladeneis")
            self.assertIsNotNone(peas)
            self.assertEqual(peas.name_de, "Erbsen")
